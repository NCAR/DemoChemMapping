!> Wrapper for the FASTJ photolysis module
!!
module photolysis_fastj
  ...
  ! FASTJ photolysis rate ids

  !> ethene + hv (CH2=CH2)
  integer, parameter :: RATE_ETHENE   = 1
  !> propene + hv (CH3-CH2-CH=CH2)
  integer, parameter :: RATE_BUTENE   = 2
  !> cyclohexe + hv (six carbon ring with one double bond)
  integer, parameter :: RATE_CYCLOHEXENE = 3

  !> Photolysis rates from FASTJ [s]
  real, dimension(NUM_RATE) :: fastj_photolysis_rates__s
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine calculate_rates_qx5( config_id, photolysis_rates__s )
    ...
    ! call the actual FASTJ to update fastj_photolysis_rates__s
    ! call FASTJ( fastj_photolysis_rates__s )

    write(*,*) "Calculating FASTJ photolysis for QX5"

    select case( config_id )
      case( CONFIG_QX5_FASTJ_MARGE )
        ! With MARGE, most of BIGALK is isoprene and monoterpenes
        ! but a rough approximation is made based on cyclohexene photolysis
        ! (this is all made up but representative of the type of logic used
        !  to build maps)
        photolysis_rates__s( GAS_RXN_BIGALK_1 ) =                             &
          0.8 * fastj_photolysis_rates__s( RATE_CYCLOHEXENE )
      ...
      case( CONFIG_QX5_FASTJ_ARES )
        ...
        !  to build maps)
        photolysis_rates__s( GAS_RXN_BIGALK_1 ) =                             &
          0.72 * fastj_photolysis_rates__s( RATE_BUTENE )
        photolysis_rates__s( GAS_RXN_BIGALK_2 ) =                             &
          1.2 * fastj_photolysis_rates__s( RATE_ETHENE )
    end select

    ! ethene is ethene
    photolysis_rates__s( GAS_RXN_ETHENE ) =                                   &
      fastj_photolysis_rates__s( RATE_ETHENE )
    ! propene scales with butene
    photolysis_rates__s( GAS_RXN_PROPENE ) =                                  &
      1.16 * fastj_photolysis_rates__s( RATE_BUTENE )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine calculate_rates_qxz( config_id, photolysis_rates__s )
    ...
    write(*,*) "Calculating FASTJ photolysis for QXZ"
    ...
    select case( config_id )
      case( CONFIG_QX5_FASTJ_MARGE )
        ...
        ! but in this photlysis model we don't have isoprene and monoterpene
        ! photlysis, so we scale the rates based on butene and cyclohexene
        ! (this is all made up but representative of the type of logic used
        !  to build maps)
        photolysis_rates__s( GAS_RXN_TALK ) =                                 &
          0.24 * fastj_photolysis_rates__s( RATE_BUTENE ) +                   &
          0.7 * fastj_photolysis_rates__s( RATE_CYCLOHEXENE )
        photolysis_rates__s( GAS_RXN_BIGALK ) =                               &
          0.09 * fastj_photolysis_rates__s( RATE_BUTENE ) +                   &
          0.43 * fastj_photolysis_rates__s( RATE_CYCLOHEXENE )
        ! there are no IALK species in MARGE (maybe we assume a rate based on
        ! butene photolysis)
        photolysis_rates__s( GAS_RXN_IALK ) =                                 &
          0.8 * fastj_photolysis_rates__s( RATE_BUTENE )
      case( CONFIG_QX5_FASTJ_ARES )
        ...
        !  to build maps)
        photolysis_rates__s( GAS_RXN_IALK ) =                                 &
          0.32 * fastj_photolysis_rates__s( RATE_BUTENE ) +                   &
          0.6 * fastj_photolysis_rates__s( RATE_ETHENE )
        ! Butene is the only 1<C<6 terminal alkene in ARES, so we get lucky
        photolysis_rates__s( GAS_RXN_TALK ) =                                 &
          fastj_photolysis_rates__s( RATE_BUTENE )
        ! There are no BIGALK species in ARES, so we scale them based on
        ! butene
        photolysis_rates__s( GAS_RXN_BIGALK ) =                               &
          0.6 * fastj_photolysis_rates__s( RATE_BUTENE )
    end select
